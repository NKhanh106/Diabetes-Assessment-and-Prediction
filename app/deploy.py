"""
Streamlit web application for Diabetes Assessment and Prediction
"""
...
with st.form(key="inform"):
    ...
    if submit_button:
        ...
        missing_fields = [key for key, value in form_values.items() if value is None or value == ""]
        
        if missing_fields:
            st.warning(f"Vui lòng điền đầy đủ thông tin! Các trường còn thiếu: {', '.join(missing_fields)}")
            ...
            try:
                ...
                st.success(f"**Kết quả dự đoán:** {answer}")
                st.info("⚠️ Lưu ý: Đây chỉ là dự đoán từ mô hình máy học. Vui lòng tham khảo ý kiến bác sĩ để có chẩn đoán chính xác.")
            
            except Exception as e:
                st.error(f"Lỗi khi dự đoán: {str(e)}")
                st.info("Vui lòng kiểm tra lại dữ liệu đầu vào hoặc liên hệ quản trị viên.")
                import traceback
